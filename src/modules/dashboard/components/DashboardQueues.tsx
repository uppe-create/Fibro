  ...
  return (
    <>
      {permissions.canIssueRegistration && permissions.canPrintCarteirinha && batchPrintableRegistrations.length > 0 && (
        <div className="cipf-panel p-6 sm:p-8">
          <div className="mb-6 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
            ...
            <Button type="button" onClick={onBatchPrint}>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {batchPrintableRegistrations.slice(0, 9).map((reg) => (
              <label key={reg.id} className="flex cursor-pointer items-start gap-3 rounded-2xl border border-[#e3e9ef] bg-[#f8fafc] p-4 transition hover:border-[#7b2cbf] hover:bg-white">
                <input
                  type="checkbox"
                ...
                <span className="min-w-0">
                  ...
                  <span className="block text-xs text-[#617184]">{maskCpf(reg.cpf)} • validade {reg.expiryDate || '-'}</span>
                </span>
              </label>
            ))}
          </div>
      ...
      <div className={`grid grid-cols-1 gap-8 ${showDocumentIssues ? 'xl:grid-cols-2' : ''}`}>
        <div className="cipf-panel p-6 sm:p-8 lg:p-10">
          ...
          <div className="space-y-5">
            {operationalQueue.map((item) => {
              ...
              return (
                <div key={reg.id} className="grid gap-5 rounded-xl border border-[#e3e9ef] bg-[#f8fafc] p-5 lg:grid-cols-[minmax(0,1fr)_minmax(280px,auto)] lg:items-center">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-bold text-[#17324d]">{reg.fullName}</p>
                    <p className="mt-1 text-xs leading-5 text-[#617184]">{maskCpf(reg.cpf)} • {getStatusLabel(reg.status)} • Validade {reg.expiryDate || '-'}</p>
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:min-w-[360px]">
                    <Button type="button" variant="outline" onClick={() => onOpenDetails(reg)} className="h-10 rounded-xl px-3 text-sm">Ver ficha</Button>
                    ...
                    {permissions.canRegisterPatientContact && ['approved', 'issued'].includes(status) && <Button type="button" variant="outline" onClick={() => onRegisterNotice(reg)} className="h-10 rounded-xl px-3 text-sm">Registrar aviso</Button>}
        ...
        {showDocumentIssues && <div className="cipf-panel p-6 sm:p-8 lg:p-10">
          <div className="mb-8">
          ...
          <div className="space-y-5">
            {documentIssueQueue.map((reg) => {
              ...
              return (
                <div key={reg.id} className="rounded-xl border border-amber-200 bg-amber-50/70 p-5">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button type="button" variant="outline" onClick={() => onOpenDetails(reg)} className="h-9 rounded-xl border-amber-300 bg-white">Conferir</Button>
                      {permissions.canEditRegistration && <Button type="button" onClick={() => onResolveIssue(reg, issues[0])} className="h-9 rounded-xl bg-amber-600 text-white hover:bg-amber-700">Resolver</Button>}
                    </div>
                  </div>
                </div>
              );
