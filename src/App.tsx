    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 animate-in fade-in zoom-in duration-500">
        <div className="bg-white/70 backdrop-blur-xl p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/20 max-w-md w-full text-center">
          ...
          <p className="text-[#86868B] mb-6">Sua conta não tem permissão para acessar o sistema. Contate um administrador.</p>
          <Button variant="outline" onClick={() => useAppStore.getState().logout()} className="w-full rounded-xl">
...
export default function App() {
  const { isAuthReady, currentUser, logout, activeTab, setActiveTab } = useAppStore();
    ...
    <div className="min-h-screen bg-[#F5F5F7] text-[#1D1D1F] font-sans selection:bg-blue-200">
      <main className="container mx-auto px-4 py-10 max-w-6xl">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <div className="flex flex-col md:flex-row justify-center items-center mb-10 print:hidden gap-4 relative">
            ...
            <div className="flex justify-end items-center gap-4 md:absolute md:right-0">
              {currentUser && (
                <>
                  <Notifications />
